...
import {target, load_From_localStorage_target} from "./target.js"
...
function handler_today_target_input () {
    const target_calories_input = document.querySelector('.js_target_calories_input');
    target_calories_input.addEventListener('keypress', (event) => {
        if (event.key === 'Enter') {
            update_target_calories();
...
function generate_target_bar_html () {
    const a = (target.target_calories) - mg_to_g(calculate_total_calories());
    console.log(`a: ${a}`);
    const html = `
    <div class="today_target">
    Today target:
    <input class="js_target_calories_input" placeholder="${target.target_calories}">
    </div>
...
function generate_total_consume_html () {
    ...
    const html = 
    `
        <div class="carbohydrates_consume">Carbohydrates:<input class="js_target_carbohydrates_input" placeholder="${ate_carbo}/${target.target_carbohydrates}--${calculate_percentage(ate_carbo,target.target_carbohydrates)}"></div>
        <div class="protein_consume">Protein:<input class="js_target_protein_input" placeholder="${ate_protein}/${target.target_protein}--${calculate_percentage(ate_protein,target.target_protein)}"></div>
        <div class="fat_consume">Fat:<input class="js_target_fat_input" placeholder="${ate_fat}/${target.target_fat}--${calculate_percentage(ate_fat,target.target_fat)}"></div>
    `
    return html;
}
